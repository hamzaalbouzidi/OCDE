...
AS
	...
	SELECT 
      ...
      ,[EventLogID]
		  ...
		  from [stg].[STG_ILibraryLog_TMP2]) SA
		   LEFT OUTER JOIN [DWH_UsageStat].[dwh].Expression_DM E on SA.DOI= E.DOI
		   LEFT OUTER JOIN [DWH_UsageStat].[dwh].FORMAT_DM F on SA.MimeType=F.MimeType where E.DOI is not null and E.DOI<>'') SA2
		   LEFT OUTER JOIN [DWH_UsageStat].[dwh].Manifestation_DM M ON SA2.Format_ID=M.Format_ID and SA2.Expression_ID=M.Expression_ID where M.IsCurrent=1) SA3

	LEFT OUTER JOIN [DWH_UsageStat].[dwh].Registration_DM
	...
	LEFT OUTER JOIN [DWH_UsageStat].[dwh].Session_DM
